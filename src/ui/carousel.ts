...
import {searchAssetsForDate, fetchAssetThumbnail, isRenderableAsset} from "../immich/api";
...
class CarouselCleanup extends Component {
...
export function registerCarouselProcessor(plugin: ImmichDailyCarouselPlugin) {
	plugin.registerMarkdownCodeBlockProcessor("immich-carousel", async (source, el, ctx) => {
		if (!hasImmichCredentials(plugin.settings)) {
			renderMessage(el, "Set your Immich base URL and API key in settings to load photos.");
			return;
		}
		...
		let assets;
		try {
			assets = await searchAssetsForDate(plugin.settings, startIso, endIso);
		} catch (error) {
			renderMessage(el, "Immich request failed. Check your settings and server availability.");
			console.error("Immich carousel search failed", error);
			return;
		}

		const visibleAssets = assets.filter(asset => isRenderableAsset(asset));
		if (visibleAssets.length === 0) {
			renderMessage(el, `No assets found for ${formatDateForDisplay(date)}.`);
			return;
		}

		renderCarousel(plugin, el, ctx, visibleAssets, formatDateForDisplay(date));
	});
}
...
async function renderCarousel(
	...
	wrapper.style.setProperty("--immich-carousel-thumb-size", `${plugin.settings.thumbnailSizePx}px`);
	const header = wrapper.createDiv({cls: "immich-carousel-header"});
	header.setText(`Immich • ${label} • ${assets.length} items`);

	const webBaseUrl = resolveWebBaseUrl(plugin.settings.baseUrl, plugin.settings.webBaseUrl);
	const assetTemplate = plugin.settings.assetUrlTemplate.trim().length > 0
		? plugin.settings.assetUrlTemplate.trim()
		: `${webBaseUrl}/photos/{{assetId}}`;
	const dayTemplate = plugin.settings.dayUrlTemplate.trim().length > 0
		? plugin.settings.dayUrlTemplate.trim()
		: `${webBaseUrl}/photos?timelineDate={{date}}`;

	if (webBaseUrl.length > 0) {
		const dayUrl = applyTemplate(dayTemplate, {baseUrl: webBaseUrl, date: label});
		const link = header.createEl("a", {cls: "immich-carousel-day-link", text: "Open in Immich"});
		link.href = dayUrl;
		link.target = "_blank";
		link.rel = "noopener";
	}
	...
	const status = wrapper.createDiv({cls: "immich-carousel-status", text: "Loading photos…"});

	const objectUrls: string[] = [];
	const cleanupFns: Array<() => void> = [() => objectUrls.forEach(url => URL.revokeObjectURL(url))];
	...
	const updateScrollState = () => {
		...
		carousel.toggleClass("immich-carousel-can-scroll-right", canScrollRight);
	};
	...
	cleanupFns.push(() => track.removeEventListener("scroll", updateScrollState));

	ctx.addChild(new CarouselCleanup(cleanupFns));
	...
	await mapWithConcurrency(assets, 4, async asset => {
		try {
			...
			const assetUrl = webBaseUrl.length > 0
				? applyTemplate(assetTemplate, {baseUrl: webBaseUrl, assetId: asset.id})
				: "";
			const container = assetUrl.length > 0
			...
			failed += 1;
			const item = track.createDiv({cls: "immich-carousel-item immich-carousel-item-error"});
